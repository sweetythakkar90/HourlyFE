...
namespace HourlyFE.Controllers
{
    public class CompanyController : Controller
    {
        ...
        public async Task<ActionResult> Register(EmployeeModel Employee)
        {
            List<EmployeeModel> EmployeeInfo = new List<EmployeeModel>();
            using (var client = new HttpClient())
            {
                ...
                var employee = new StringContent(JsonConvert.SerializeObject(Employee));

                // Define request data format
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                
                //Sending request to find web api REST service resource GetAllComapnies using HttpClient 
                HttpResponseMessage Res = await client.PostAsync("api/employees", employee);
